...
class HighlightCog(commands.Cog):
    ...
    async def show(self, ctx):
        ...
        e = discord.Embed(title=f"🔍 {username}'s Highlight List 🔍", color=quake_color)
    ...
    async def help(self, ctx):
        try:
            if not await self.has_permission(ctx.author, ctx.guild):
                await ctx.send("You don't have the required permissions for this command!", ephemeral=True)
                return
            e = discord.Embed(title=f"⚙️ Highlight Help ⚙️", color=quake_color)
            e.description = "Here are the available public commands for managing highlights. \n### ❓ What does Highlights do ❓ \n'Highlight' commands help with moderation! Each time a word in your 'highlight list' is mentioned, it will **DM** you with information about the user and a link to the message! \n\n### 📌 Highlight Commands 📌 \n> ⚙️ `/highlight help` **|** Sends the Highlight Help Menu \n> 📑 `/highlight defaults` **|** Adds a default list of slurs to your highlight list \n> ➕ `/highlight add [word]` **|** Adds a word to your highlight list \n> ➖ `/highlight remove [word]` **|** Removes a word from your highlight list \n> ♻️ `/highlight clear` **|** Clears your highlight list \n> 🔕 `/highlight ignore [word]` **|** Ignores a word that contains a word in your highlight list **Ex:** If yuou ignore '**hi**', you won't get DM'd when someone says '**hit**'. \n> 🔔 `/highlight unignore [word]` **|** Unignores a word that contains a word in your highlight list \n> ❌ `/highlight block [item]` **|** [`item` can either be a channel mention or user mention], when a user or channel is 'blocked' you won't get DM'd when they say a word in your highlight list \n> ✅ `/highlight unblock [item]` **|** Unblocks a user or channel you had blocked [`item` can either be a channel mention or user mention]\n> 📰 `/highlight show` **|** Shows all the information about your highlight list"
            await ctx.send(embed=e, ephemeral=True)
        ...
        for user_id in user_ids:
            ...
            for word in words:
                if word in content and not any(ignored_word in content for ignored_word in ignored_words):
                    if user:
                        e = discord.Embed(title=f"🚨 Word Mentioned 🚨", color=quake_color, timestamp=datetime.utcnow())
